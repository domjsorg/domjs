...
function buildLib() {
  ...
  intializerFile('domInitializer.js');
...
function buildLibDeps() {
  ...
  intializerFile('domInitializer-deps.js');
...
function intializerFile(intializerFile) {
  ...
  const domFiles = fs.readdirSync(`${config.SRC_DIR}/domjs`); 
    ...
    if (fs.lstatSync(component).isDirectory() && !file.startsWith('_')) {
      ...
      componentFiles.forEach(filejs => {
        if (filejs.endsWith(".js")) {
          const filepath = `../domjs/${componentName}/${filejs}`;
